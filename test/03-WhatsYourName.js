...
describe("03 - What's Your Name?", () => {
    ...
    beforeEach(() => {
        ...
        paragraphs = React.addons.TestUtils.scryRenderedDOMComponentsWithTag(
            component,
            'p'
        );
        inputs = React.addons.TestUtils.scryRenderedDOMComponentsWithTag(
            component,
            'input'
        );
    });
        ...
        describe('Task #1 - greet user', () => {
            it('Should change name in paragraph on name change in input', () => {
                ...
                assert.equal(
                    paragraphs.length,
                    1,
                    'There should be only one `p` element rendered by the component'
                );
                assert.equal(
                    inputs.length,
                    1,
                    'There should be only one `input` rendered by the component'
                );
                ...
                assert.equal(
                    nameInParagraphEqualsTo(paragraph, 'XYZ'),
                    true,
                    "After changing name in input, I should see the change in `p` element's content. In other words: `this.state.name` should change."
                );
                ...
                assert.equal(
                    nameInParagraphEqualsTo(paragraph, 'ZYX'),
                    true,
                    'After changing name in input for the second time, we should see the change in `p` element. In other words: `this.state.name` should change.'
                );
            });
        });
